...
This backend server implements a one-to-many broadcast system using **mediasoup** (WebRTC SFU), **Socket.IO** for real-time communication, and **Express** as the web server. The server acts as a Selective Forwarding Unit (SFU), routing media streams from broadcasters to multiple viewers.
