...
/**
 * @param {string|null} sessionId — BroadcastSession.id (Phase 8 scope)
 * @param {boolean}     enabled — set true only for the classroom owner-teacher
 */
export default function useTeacherQueue(sessionId, enabled) {
  ...
  /* Reset queue when session changes — new lecture starts with empty queue. */
  useEffect(() => {
    setQueue([]);
    setError(null);
  }, [sessionId]);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const res = await authFetch(
        `${API_URL}/api/chat/sessions/${sessionId}/queue`,
      );
      if (!res.ok) {
  ...
  }, [enabled, sessionId, token, API_URL, authFetch, sortQueue]);
  ...
  const fetchOne = useCallback(
    async (messageId) => {
      try {
        const res = await authFetch(
          `${API_URL}/api/chat/sessions/${sessionIdRef.current}/messages?limit=100`,
        );
  ...
  useEffect(() => {
    ...
    const onStatus = async ({ messageId, status, sessionId: msgSessionId }) => {
      // Drop stale events from a previous session that might still be in flight.
  ...
  }, [enabled, sessionId, token, fetchOne, sortQueue]);
